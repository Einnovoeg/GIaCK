...
public class Wine {
    ...
    public static func runProgram(
        ...
        if bottle.settings.dxvk && GIACKWineInstaller.hasDXVKRuntime() {
    ...
    /// Requests a graceful shutdown without blocking application termination.
    public static func killBottle(bottle: Bottle) {
        Task.detached(priority: .userInitiated) {
            do {
                _ = try await runWineserver(["-k"], bottle: bottle)
            } catch {
                logger.error("Failed to stop bottle: \(error.localizedDescription, privacy: .public)")
            }
        }
    }
    ...
    private static func constructWineEnvironment(
        ...
        if !GIACKWineInstaller.hasDXVKRuntime() {
